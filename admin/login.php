<?php
require_once __DIR__ . '/../core/LH/Controllers/AuthController.php';
...
<main class="w-full max-w-xs" role="main" aria-label="Login form">
    <div class="flex items-center justify-center mb-6">
        ...
        <span class="text-[#0B1C3F] text-lg font-normal" id="blog-title">BLOG</span>
    </div>
    <?php if ($error): ?>
        <div class="text-red-500 text-xs mb-4"><?= htmlspecialchars($error) ?></div>
    <?php endif; ?>
    <form class="space-y-4" method="POST" action="login.php" aria-labelledby="blog-title">
        <div>
            <label class="block text-xs text-[#0B1C3F] mb-1 font-normal" for="username">Username</label>
            <input aria-required="true" name="username" class="w-full border border-gray-200 rounded-md px-3 py-2 text-xs placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-purple-600 focus:border-purple-600" id="username" placeholder="Enter your username" type="text" />
        </div>
        <div>
            <label class="block text-xs text-[#0B1C3F] mb-1 font-normal" for="password">Password</label>
            <input aria-required="true" name="password" placeholder="Enter your password" class="w-full border border-gray-200 rounded-md px-3 py-2 text-xs placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-purple-600 focus:border-purple-600" id="password" type="password"/>
        </div>
        <button aria-label="Login to your account" class="w-full bg-purple-600 text-white text-xs rounded-md py-2" type="submit">Login</button>
    </form>
</main>
</body>
</html>
